...
/**
 * Menampilkan grid lampiran catatan (foto, PDF, Excel).
 * Foto -> klik buka preview modal. PDF/Excel -> klik langsung buka/download di tab baru.
 */
    ...
    <>
          ...
          <div key={a.id} className="relative group">
            {a.fileKind === "image" ? (
              ...
              <a
                ...
                title={a.filename}
              >
                <span className={`text-xs font-semibold ${KIND_CHIP[a.fileKind]?.className ?? "text-espresso-soft"}`}>
                  {KIND_CHIP[a.fileKind]?.label ?? "FILE"}
                </span>
      ...
      {active && (
        ...
        >
          ...
          >
            <img src={active.url} alt={active.filename} className="max-h-[70vh] w-auto rounded-xl object-contain" />
